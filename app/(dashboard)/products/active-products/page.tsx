...
export default async function ActiveProductsPage({
  ...
  return (
    ...
    >
      <ActiveProductsContent searchParams={searchParams} />
    </Suspense>
  );
}

async function ActiveProductsContent({
  searchParams,
}: {
  searchParams: Promise<{ search?: string; page?: string }>;
}) {
  ...
  // Filter for only active products that are activated in database
  const activeProducts = products.filter((product) => product.isActivated);
